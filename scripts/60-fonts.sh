...
if [[ -d "${FIRACODE_FONT_DIR}" ]]; then
  ...
  run cp "${FIRACODE_DIR}"/ttf/*.ttf "${FIRACODE_FONT_DIR}/"
fi
...
run bash "${POWERLINE_DIR}/install.sh"
...
if [[ -d "${OPERATOR_DIR}/original" ]] &&
  ...
  if ! command_exists npm; then
    ...
    if ! command_exists python3 || ! python3 -c "import fonttools" 2>/dev/null; then
      run pip3 install --quiet fonttools
    fi
    if [[ "${DRY_RUN}" == "1" ]]; then
      info "[dry-run] would run: npm install --silent (in ${OPERATOR_DIR})"
      info "[dry-run] would run: bash build.sh (in ${OPERATOR_DIR})"
    else
      (
        cd "${OPERATOR_DIR}"
        npm install --silent
